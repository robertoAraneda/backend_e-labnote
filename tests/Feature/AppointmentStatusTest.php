...
use App\Models\Role;
use App\Models\User;
use Database\Seeders\AppointmentStatusPermissionsSeeder;
...
class AppointmentStatusTest extends TestCase
{
    ...
    const BASE_URI = '/api/v1/appointment-statuses';
    ...
    public function setUp(): void
    {
        ...
        $this->seed(AppointmentStatusPermissionsSeeder::class);

        $role = Role::where('name', 'Administrador')->first();

        $user->assignRole($role);

        $modelClass = new AppointmentStatus();

        $this->user = $user;
        $this->role = $role;
        $this->model = AppointmentStatus::factory()->create();
        $this->perPage = $modelClass->getPerPage();
        $this->table = 'appointment_statuses';
    ...
    public function test_se_puede_obtener_una_lista_del_recurso(): void
    {
        ...
        AppointmentStatus::factory()->count(20)->create();

        $uri = sprintf('%s', self::BASE_URI);
        $countModels = AppointmentStatus::count();
    ...
    public function test_se_genera_error_http_forbidden_al_crear_un_recurso_sin_privilegios(): void
    {
        ...
        $this->role->revokePermissionTo('appointmentStatus.create');
    ...
    {
        $this->role->revokePermissionTo('appointmentStatus.update');
    ...
    {
        $this->role->revokePermissionTo('appointmentStatus.delete');
