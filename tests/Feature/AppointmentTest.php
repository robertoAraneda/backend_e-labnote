...
use Database\Seeders\AppointmentPermissionsSeeder;
...
class AppointmentTest extends TestCase
{
    ...
    public function setUp(): void
    {
        ...
        $this->seed(AppointmentPermissionsSeeder::class);

        $role = Role::where('name', 'Administrador')->first();

        $user->assignRole($role);

        $modelClass = new Appointment();

        $this->user = $user;
        $this->role = $role;
        $this->model = Appointment::factory()->create();
        $this->table = $modelClass->getTable();
        $this->base_url = '/api/v1/appointments';
    ...
    public function se_puede_crear_un_recurso(): void //store
    {
        ...
        $response->assertJson(fn(AssertableJson $json) => $json
            ...
            ->where('minutes_duration', $factoryModel['minutes_duration'])
            ->etc()
        );
    ...
    public function se_puede_modificar_un_recurso(): void // update
    {
        ...
        $this->actingAs($this->user, 'api')
            ...
            ->assertJson(fn(AssertableJson $json) => $json
                ...
                ->where('description', 'description modificado')
                ->where('minutes_duration', $this->model->minutes_duration)
                ->etc()
            );
    ...
    public function se_genera_error_http_forbidden_al_crear_un_recurso_sin_privilegios(): void
    {
        ...
        $this->role->revokePermissionTo('appointment.create');
    ...
    {
        $this->role->revokePermissionTo('appointment.update');
    ...
    {
        $this->role->revokePermissionTo('appointment.delete');
