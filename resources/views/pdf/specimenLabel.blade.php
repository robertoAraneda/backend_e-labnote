...
@php($specimens = (object) $serviceRequest['_links']['specimens']['collection'])
...
@foreach($specimens as $specimen)
    <div style="border: 1px black solid; padding: 10px 20px; width: 190px; margin-bottom: 20px;">
        <div>
            <div class="mini" style=" font-size: 12px; display: block;"><span style="background-color: black; color: white; font-weight: bold; padding:10px;">{{$patient->identifier[0]['value']}}</span> </div>
            <div class="mini" style="display: block;"><span>{{ $patient->name['given'] }} {{ $patient->name['father_family'] }} {{ $patient->name['mother_family'] }}</span></div>
